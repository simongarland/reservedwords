...
PRIM:.Q.res
PRIM,:`from`by
TYPE:`boolean`byte`short`int`long`real`float`char`symbol`month`date`datetime`time`minute`second`timestamp`timespan
DATE:`year`month`week`mm`dd
TIME:`hh`minute`second`mm`ss`uu
QNAMES:asc key .q
QNAMES:QNAMES except `
ZNAMES:asc distinct`.z.i`.z.d`.z.D`.z.t`.z.T`.z.f`.z.x`.z.a`.z.h`.z.u`.z.w`.z.z`.z.Z`.z.k`.z.l`.z.o`.z.b`.z.s`.z.pg`.z.pi`.z.ps`.z.po`.z.pc`.z.ph`.z.pp`.z.ts`.z.vs`.z.pw`.z.exit`.z.W`.z.p`.z.n`.z.P`.z.N`.z.ac
